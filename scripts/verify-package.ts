...
try {
  ...
  assert.doesNotMatch(
    ...
    /@revisium\/revo-pipeline|CompiledPipeline|PipelineFacts|PipelineDecision|\b(?:RunStore|ExecutionPlanSource|ExecutorResolver|ManagerIdSource|LifecycleAttemptAuthority)\b|\/(?:storage|ports|pipeline)\//,
    'Packed root declarations must remain private-port, storage, and pipeline-package-free',
  );
