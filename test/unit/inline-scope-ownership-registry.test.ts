...
describe('inline logical scope ownership', () => {
  ...
  it('exposes registered ids, directive acceptance, and settlement', () => {
    const registry = registryWithReadyChildren();

    expect(registry.registeredWorkflowIds()).toEqual([
      rootWorkflowId,
      childWorkflowId,
      otherWorkflowId,
    ]);
    expect(registry.acceptsDirective(childWorkflowId)).toBe(true);
    expect(registry.isSettled(childWorkflowId)).toBe(false);

    registry.finish(childWorkflowId);
    expect(registry.acceptsDirective(childWorkflowId)).toBe(false);

    registry.settle(childWorkflowId);
    expect(registry.isSettled(childWorkflowId)).toBe(true);
    expect(registry.acceptsDirective(childWorkflowId)).toBe(false);
  });
});
